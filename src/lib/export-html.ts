...
function blockHtml(node: JSONContent, ctx: Ctx): string {
  switch (node.type) {
    ...
    case 'htmlBlock': {
      ...
      return `<div class="x-embed"><div class="x-embed-head"><span>${name}</span><button class="x-embed-expand" title="Expand">⤢</button></div><iframe sandbox="allow-scripts allow-popups allow-popups-to-escape-sandbox" style="height:${height}px" srcdoc="${escAttr(withBaseTarget(media.text))}"></iframe></div>`
    }
    case 'databaseBlock': {
...
.x-ref{border-radius:2.5px;transition:background .15s ease}
.x-ref.is-live{background:var(--hl);box-shadow:0 0 0 2px var(--hl)}
